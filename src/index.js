...
async function fetchImages(searchQuery) {
  const response = await axios.get(
    `${BASE_URL}?key=${KEY}&q=${searchQuery}&image_type=photo&orientation=horizontal&safesearch=true&per_page=40&page=${pageCount}`
  );
  return response;
}
...
function onImageSearch(event) {
  ...
  if (searchQuery === '')
    return Notify.failure('The search field can not be empty! Try again!', {
